...
import {IDatastore, ITransactionStatus, IResult} from "../core/model/interfaces";
...
export class services implements IDatastore {
    ...
    public create(record: ITransactionStatus): Promise<IResult> {
    ...
    public update(record: ITransactionStatus): Promise<IResult> {
        return new Promise((resolve, rejects) => {
            const params = {
                TableName: this.tableName,
                Key: {
